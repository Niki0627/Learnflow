...
${String(lecture.content || "").slice(0, 10000)}`;
    let cards = [];
    try {
      const result = await generateAIContent(prompt, { maxTokens: Math.min(5000, count * 220) });
    ...
    if (!cards.length) {
      cards = Array.from({ length: count }, (_, i) => ({
        ...
        back: String(lecture.content || lecture.title).slice(i * 120, i * 120 + 240) || lecture.title,
      }));
    }
