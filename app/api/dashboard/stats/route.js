...
export async function GET(request) {
  try {
    ...
    const [
      ...
      supabase.from("lecture_notes").select("id,title,subject,created_at").eq("user_id", user.id),
      supabase.from("flashcards").select("id", { count: "exact", head: true }).eq("user_id", user.id),
      supabase.from("quiz_attempts").select("score,total_questions,completed_at,lecture_notes(subject,title)").eq("user_id", user.id),
    ...
    for (const lecture of lectureRows) {
      ...
      if (!subjectBuckets.has(subject)) subjectBuckets.set(subject, { subject, lectures: 0, score: 0, total: 0 });
    ...
    const scoreDistribution = [
      { name: "Excellent (>90%)", value: attempts.filter((a) => ((a.score || 0) / Math.max(a.total_questions || 1, 1)) >= 0.9).length },
      { name: "Good (70-90%)", value: attempts.filter((a) => {
        const pct = (a.score || 0) / Math.max(a.total_questions || 1, 1);
        return pct >= 0.7 && pct < 0.9;
      }).length },
      { name: "Average (50-70%)", value: attempts.filter((a) => {
        const pct = (a.score || 0) / Math.max(a.total_questions || 1, 1);
        return pct >= 0.5 && pct < 0.7;
      }).length },
      { name: "Weak (<50%)", value: attempts.filter((a) => ((a.score || 0) / Math.max(a.total_questions || 1, 1)) < 0.5).length },
    ];
    ...
    return Response.json({
      ...
      quiz_scores: attempts.slice(-10).map((attempt, index) => ({
        name: `Quiz ${index + 1}`,
        score: Math.round(((attempt.score || 0) / Math.max(attempt.total_questions || 1, 1)) * 100),
      })),
