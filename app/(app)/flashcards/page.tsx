...
import type { Flashcard, Lecture } from "@/src/lib/types";
...
export default function FlashcardsPage() {
  ...
  useEffect(() => {
    api.get<Lecture[]>("lectures/")
      .then(d => setLectures(Array.isArray(d) ? d : []))
      .catch(() => setError("Failed to load lectures."))
      .finally(() => setLoading(false));
  }, []);

  useEffect(() => {
    if (!selectedLecture) { setFlashcards([]); return; }
    setLoading(true);
    api.get<Flashcard[]>(`flashcards/?note_id=${selectedLecture}`)
      .then(d => { setFlashcards(d ?? []); setCurrentIndex(0); setIsFlipped(false); setSessionComplete(false); setRatings({}); })
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const d = await api.get<Flashcard[]>(`flashcards/?note_id=${selectedLecture}`);
      setFlashcards(d ?? []);
    } catch (err: unknown) {
  ...
  const handleRate = async (rating: string) => {
    ...
    try { if (card.id) await api.post(`flashcards/${card.id}/review/`, { rating }); } catch { /* keep local */ }
    handleNext();
  };
        ...
        <div className="flex-1 rounded-[2rem] border border-primary/20 bg-card p-6 md:p-10 shadow-sm flex flex-col items-center justify-center min-h-[500px]">
          {flashcards.length === 0 ? (
            ...
            <div className="text-center max-w-sm animate-in zoom-in-95 duration-500">
              ...
              <button
                onClick={() => { setCurrentIndex(0); setIsFlipped(false); setRatings({}); setSessionComplete(false); }}
            ...
            <div className="w-full max-w-2xl flex flex-col h-full">
              ...
              {/* Card */}
              <div
                className="relative flex-1 cursor-pointer min-h-[300px]"
                onClick={() => setIsFlipped(!isFlipped)}
              >
                <div className={cn(
                  "absolute inset-0 flex h-full w-full flex-col items-center justify-center rounded-[2rem] border-2 p-10 text-center shadow-lg transition-all duration-300",
