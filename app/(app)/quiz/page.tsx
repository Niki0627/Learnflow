...
export default function QuizEntryPage() {
  ...
  useEffect(() => {
    api.get<Lecture[] | { results: Lecture[] }>("lectures/")
      .then(data => setLectures(Array.isArray(data) ? data : (data as { results: Lecture[] }).results ?? []))
      .catch(() => setLectures([]))
  ...
  const startQuiz = () => {
    ...
    setLoading(true);
    // Pass timer duration via searchParams so the quiz page can read it
          ...
          <div className="rounded-3xl border bg-card p-8 shadow-sm">
            <div className="mb-6 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/10 text-primary"><BookOpen size={20} /></div>
                <div>
            ...
            <div className="max-h-[400px] overflow-y-auto space-y-3 pr-1">
              {fetchingLectures ? (
                ...
                lectures.map(lec => {
                  const selected = selectedNoteIds.includes(lec.id);
                  return (
                    <div
                      key={lec.id}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-sm truncate text-foreground">{lec.title ?? `Lecture ${lec.id}`}</p>
                        {lec.subject && <p className="text-xs text-muted-foreground">{lec.subject}</p>}
                      </div>
                    </div>
                  );
                })
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="rounded-3xl border bg-card p-8 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-500/10 text-violet-500"><Brain size={20} /></div>
              <div>
          ...
          <div className="rounded-3xl border bg-card p-8 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-500/10 text-amber-500"><Clock size={20} /></div>
              <div>
