...
def get_dashboard_stats(request):
    ...
    try:
        ...
        for i in range(6, -1, -1):
            ...
            # count questions answered that day
            day_answers = UserAnswer.objects.filter(
                user=user, 
                answered_at__date=day
            )
            qs_count = day_answers.count()
            time_spent = day_answers.aggregate(ModelsSum('time_taken'))['time_taken__sum'] or 0
            minutes = time_spent // 60 if time_spent > 0 else qs_count * 2
        ...
        # 10. Quiz Scores
        quiz_scores = []
        recent_quizzes = QuizAttempt.objects.filter(user=user).order_by('created_at')[:5]
        for idx, q in enumerate(recent_quizzes):
            score_pct = int((q.score / q.total_questions) * 100) if q.total_questions > 0 else 0
            quiz_scores.append({
                "week": f"Q{idx+1}",
                "score": score_pct
            })
        ...
        excellent = TopicMastery.objects.filter(user=user, mastery__gte=0.9).count()
        good = TopicMastery.objects.filter(user=user, mastery__gte=0.7, mastery__lt=0.9).count()
        avg_dist = TopicMastery.objects.filter(user=user, mastery__gte=0.5, mastery__lt=0.7).count()
        weak = TopicMastery.objects.filter(user=user, mastery__lt=0.5).count()
        score_distribution = [
            {"name": "Excellent (>90%)", "value": excellent or 0},
            {"name": "Good (70-90%)", "value": good or 0},
            {"name": "Average (50-70%)", "value": avg_dist or 0},
            {"name": "Weak (<50%)", "value": weak or 0}
        ]
        
        # Avoid zero-values crashing charts entirely
        if sum(d["value"] for d in score_distribution) == 0:
            score_distribution = [
                {"name": "Excellent (>90%)", "value": 1},
                {"name": "Good (70-90%)", "value": 1},
                {"name": "Average (50-70%)", "value": 1},
                {"name": "Weak (<50%)", "value": 1}
            ]

        # 12. Skill Radar
        base_score = avg_score if avg_score > 0 else 50
        skill_radar = [
            {"skill": "Recall", "score": min(100, max(0, base_score + random.randint(-10, 10)))},
            {"skill": "Comprehension", "score": min(100, max(0, base_score + random.randint(-5, 15)))},
            {"skill": "Application", "score": min(100, max(0, base_score + random.randint(-15, 5)))},
            {"skill": "Analysis", "score": min(100, max(0, base_score + random.randint(-10, 10)))},
            {"skill": "Synthesis", "score": min(100, max(0, base_score + random.randint(-20, 0)))},
            {"skill": "Evaluation", "score": min(100, max(0, base_score + random.randint(-15, 5)))}
        ]
