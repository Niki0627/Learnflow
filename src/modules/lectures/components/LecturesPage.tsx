...
import type { Lecture, LectureQuestion, LectureDetails } from "../types";
...
function NoPreviewFallback({ lecture, isLegacy, onReuploadSuccess }: {
  ...
  const handleReupload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await reuploadPDF(lecture.id, file);
...
function FileViewer({ lecture }: { lecture: Lecture | null }) {
  ...
  if (!lecture.file || !canPreview) {
    ...
    return <NoPreviewFallback lecture={lecture} isLegacy={isLegacy} onReuploadSuccess={() => window.location.reload()} />;
  }
        ...
        {previewError ? (
          ...
          >
            <div className="relative group">
              <Page pageNumber={Math.min(pageNumber, numPages || pageNumber)} scale={scale} renderTextLayer renderAnnotationLayer={false} className="shadow-2xl shadow-black/40 rounded-sm overflow-hidden border border-white/10" />
            </div>
          </Document>
        ) : fileType === "image" ? (
            ...
            <div className="flex h-full flex-col overflow-hidden">
              {tab === "questions" && (
                <div className="flex-1 overflow-y-auto p-6">
                  ...
                  <div className="space-y-4">
                    {details?.questions?.map((q: LectureQuestion, idx) => (
                      <div key={q.id} className="rounded-2xl border bg-card p-5 shadow-sm">
                        ...
                        <div className="space-y-2">
                          {(["A", "B", "C", "D"] as const).map(opt => {
                            const key = `option_${opt.toLowerCase()}` as keyof LectureQuestion;
...
function UploadPanel({ onUploadSuccess }: { onUploadSuccess: () => void }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      if (uploadTab === "text") {
        await uploadTextNote(title, content);
      } else {
        await uploadPDF(title, pdfFile!);
      }
      ...
      {uploadDialogOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg rounded-3xl border bg-card p-6 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <UploadPanel onUploadSuccess={() => { loadLectures(); setUploadDialogOpen(false); }} />
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm rounded-3xl border bg-card p-6 shadow-2xl animate-in zoom-in-95 duration-200 text-center">
            ...
            <div className="flex gap-3">
              <button disabled={deleting} onClick={() => { setDeleteDialogOpen(false); }} className="flex-1 rounded-full bg-muted py-3 font-bold text-muted-foreground hover:bg-muted/80 disabled:opacity-50">Cancel</button>
