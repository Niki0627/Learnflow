...
export default function GenerateQuestionsPage() {
  ...
  const [lectures, setLectures] = useState<GenerateLecture[]>([]);
  ...
  useEffect(() => {
    fetchGenerateLectures().then(d => setLectures(Array.isArray(d) ? d : [])).catch(() => {});
  }, []);
  ...
  const generate = async () => {
    ...
    setLoading(true);
    try {
      const qList = await generateMCQs(noteId, Number(maxGenerate));
      setQuestions(qList);
