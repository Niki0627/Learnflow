...
export default function QuizResultPage() {
  ...
  useEffect(() => {
    ...
    fetchDashboardStats().then(d => setStreak(d.streak)).catch(() => {});
  }, [router]);
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 bg-card p-8 rounded-[2.5rem] border border-border/50 shadow-sm">
        <div>
          <div className="flex items-center gap-2 mb-3">
            <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-black uppercase tracking-wider text-primary">Completed</span>
        ...
        <div className="flex gap-3">
          <button onClick={() => setShowReview(!showReview)} className="flex items-center gap-2 rounded-xl border border-border bg-background px-5 py-2.5 text-sm font-bold text-foreground hover:bg-muted transition">
            {showReview ? <><ChevronUp size={16} /> Hide Review</> : <><ChevronDown size={16} /> Review Questions</>}
          </button>
          <button onClick={() => router.push("/dashboard")} className="flex items-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-bold text-primary-foreground shadow-md hover:bg-primary/90 transition">
              ...
              <div className="h-[250px]">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData}>
                    ...
                    <Tooltip contentStyle={{ borderRadius: "12px", border: "none", boxShadow: "0 10px 30px rgba(0,0,0,0.1)" }} formatter={(v: unknown) => [`${v}%`, "Score"]} />
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-3 text-xs">
                          ...
                          {!ans.isCorrect && <span className="text-muted-foreground">Correct: {ans.correctAnswer}</span>}
                        </div>
                        {ans.explanation && !ans.isCorrect && <p className="text-xs text-muted-foreground italic mt-2">{ans.explanation}</p>}
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <div className="flex flex-col gap-6">
          <div className="rounded-[2rem] border border-primary/20 bg-card p-6 shadow-sm space-y-4">
            ...
            <div className="rounded-2xl p-4 border border-amber-500/20 bg-amber-500/10">
              ...
              {data.total - data.score > 0 && (
                <button onClick={() => router.push("/study-plan")} className="mt-2 text-xs font-bold text-amber-600 hover:underline">Go to Study Plan →</button>
              )}
            </div>
          </div>
                ...
                {breakdown.map(b => (
                  <div key={b.label}>
                    ...
                    <div className="h-2 rounded-full bg-muted overflow-hidden">
                      <div className="h-full rounded-full bg-primary transition-all duration-1000" style={{ width: `${b.pct}%` }} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <button onClick={() => router.push("/quiz")} className="flex items-center justify-center gap-2 rounded-2xl bg-primary px-6 py-4 font-bold text-primary-foreground shadow-md hover:bg-primary/90 transition">
            <Trophy size={18} /> Try Another Quiz
          </button>

          {/* Used to suppress unused var warning */}
          <span className={cn("hidden", gradeColor)}>{grade}</span>
        </div>
      </div>
    </div>
  );
