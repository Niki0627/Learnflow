...
import { Delete as DeleteIcon, Edit as EditIcon, ClipboardPaste as ContentPasteIcon, NotebookText as NotesIcon } from "lucide-react";
...
const NotesSidebar = ({ lectureId }: { lectureId: number | string }) => {
  ...
  useEffect(() => {
    const handler = (e: Event & { detail?: { text?: string } }) => {
      const text = e?.detail?.text || (window as any).lastSelectedPdfText;
      if (text) {
  ...
  const handleCreate = async () => {
    ...
    try {
      const data = await api.post<StickyNote>("sticky-notes/", {
  ...
  const handleSaveEdit = async (noteId: number) => {
    try {
      const data = await api.put<StickyNote>(`sticky-notes/${noteId}/`, {
        title: editTitle,
        content: editContent,
        color: editColor,
  ...
  const handleGrabText = (setContentFunc: React.Dispatch<React.SetStateAction<string>>) => {
    const selected = window.getSelection()?.toString() || (window as any).lastSelectedPdfText;
    if (selected) {
