...
export default function ConceptCoachPage() {
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      const data = await api.post<AIChatResponse | string>("/ai-tutor/chat/", { message: trimmed, chat_history: history });
      const responseText = typeof data === "string" ? data : data.response || data.message || JSON.stringify(data);
      const hints = typeof data === "string" ? [] : Array.isArray(data.hints) ? data.hints : [];
      setMessages((prev) => [...prev, { role: "assistant", content: responseText, hints, is_error: typeof data === "string" ? false : !!data.is_error, time: formatTime() }]);
    } catch (err) {
      const apiErr = err as ApiClientError;
      const errMsg = apiErr.message || (err instanceof Error ? err.message : "") || "Unknown error";
