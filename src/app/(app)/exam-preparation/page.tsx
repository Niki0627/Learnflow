...
import type { ExamSyllabus, ExamQuestion } from "@/src/lib/types";

interface MarkRow { marks: number; count: number; }
interface Strategy { overview?: string; focus_areas?: string[]; time_allocation?: string; tips?: string[]; raw?: string; }
...
export default function ExamPreparationPage() {
  ...
  useEffect(() => {
    api.get<ExamSyllabus[]>("exam/syllabi/").then(d => { setSyllabi(Array.isArray(d) ? d : []); }).catch(() => {});
  }, []);

  useEffect(() => {
    if (!activeSyllabusId) return;
    setFetchingQuestions(true);
    api.get<ExamQuestion[]>(`exam/syllabus/${activeSyllabusId}/questions/`)
  ...
  const onDrop = useCallback(async (files: File[]) => {
    ...
    try {
      ...
      const res = await api.post<ExamSyllabus>("exam/syllabus/upload/", undefined, { formData: fd });
  ...
  const generateQuestions = async () => {
    ...
    setGenerating(true);
    try {
      await api.post(`exam/syllabus/${activeSyllabusId}/generate/`, {
        mark_distribution: markRows,
        use_secure_centum: secureCentumMode,
        has_previous_papers: uploadedPapers.length > 0,
      });
      const d = await api.get<ExamQuestion[]>(`exam/syllabus/${activeSyllabusId}/questions/`);
  ...
  const generateStrategy = async () => {
    ...
    try {
      const res = await api.post<Strategy>(`exam/syllabus/${activeSyllabusId}/strategy/`, {});
      setStrategy(res);
    } catch { } finally { setGeneratingStrategy(false); }
  };

  const uploadPapers = async () => {
    if (!activeSyllabusId || uploadedPapers.length === 0) return;
    setPapersUploading(true);
    try {
      for (const paper of uploadedPapers) {
        const fd = new FormData();
        fd.append("file", paper);
        await api.post(`exam/syllabus/${activeSyllabusId}/papers/`, undefined, { formData: fd });
      }
      setUploadedPapers([]);
    } catch { } finally { setPapersUploading(false); }
  };
