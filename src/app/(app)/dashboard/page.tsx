...
import { cn, formatUsername } from "@/src/lib/utils";
import type { DashboardStats, WeakTopic } from "@/src/lib/types";
...
interface StatCardProps {
  ...
  themeColor?: "orange" | "indigo" | "emerald" | "amber";
}
...
const StatCard = ({ title, value, subtext, icon: Icon, themeColor = "indigo" }: StatCardProps) => {
...
const QuickActionCard = ({ title, subtitle, icon: Icon, href, colorClass }: QuickActionCardProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
      <div className="w-full max-w-md rounded-3xl bg-card border shadow-2xl p-6 relative animate-in zoom-in-95 duration-200">
        <button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-full hover:bg-muted text-muted-foreground transition-colors">
          <X size={18} />
        </button>
...
interface WeakTopicDialogState {
  open: boolean;
  topic: string | null;
  subject: string | null;
  noteId: number | null;
  data: WeakTopicData | null;
  loading: boolean;
}

function WeakTopicModal({ state, onClose, onRetry, onPractice }: {
  state: WeakTopicDialogState;
  onClose: () => void;
  onRetry: () => void;
  onPractice: () => void;
}) {
...
export default function DashboardPage() {
  ...
  const [weakDialog, setWeakDialog] = useState<WeakTopicDialogState>({
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => { if (!document.hidden) fetchStats(); }, 30000);
  ...
  const explainWeakTopic = async (topic: string, subject: string, noteId: number | null) => {
    ...
    try {
      ...
      const res = await api.post<{ data: WeakTopicData }>("weak-topic/explain/", { topic, subject }, { signal: controller.signal });
  ...
  return (
    <div className="flex flex-col gap-10 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {statsError && (
        <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm font-bold text-red-600">{statsError}</div>
      )}
          ...
          <div className="flex flex-col rounded-[2rem] border border-red-500/20 bg-card p-8 shadow-sm">
            ...
            <div className="flex flex-1 flex-wrap content-start gap-2 overflow-y-auto max-h-[260px]">
              {d.weak_topics.length > 0 ? (
                d.weak_topics.slice(0, 12).map((w, idx) => {
                  const isCritical = (w.accuracy ?? 0) < 40;
                  const isWarning = !isCritical && (w.accuracy ?? 0) < 70;
                  return (
                    <button
                      key={idx}
                      onClick={() => explainWeakTopic(w.topic, w.subject, w.note_id ?? null)}
                      className={cn(
                    ...
                    >
                      <span>{w.topic}</span>
                      <span className="opacity-70">{w.accuracy}%</span>
                    </button>
                  );
                })
              ) : (
